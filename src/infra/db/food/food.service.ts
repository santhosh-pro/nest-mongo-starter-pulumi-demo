...
export class FoodService {
  constructor(
    @InjectModel('Food')
    protected readonly model: Model<Food & Document>
  ) {}

  public async findAll(model: MongooseQueryModel): Promise<Food[]> {
  ...
  public async findById(id: string): Promise<Food> {
    const query = this.model.findById(this.toObjectId(id));

    return query.exec();
  }

  public async findOne(model: MongooseQueryModel): Promise<Food> {
  ...
  public async delete(id: string): Promise<any> {
    return this.model.deleteOne(this.toObjectId(id)).exec();
  }
